...
{
    cache newcache;
    newcache.s = index_bit(S);      // set_bit size
    newcache.E = E;                 // way size
    newcache.b = index_bit(b);      // block size

    newcache.sets =(cset*)malloc(sizeof(cset)* S);

    for(int i =0; i< S; i++){
        newcache.sets[i].lines = (cline*)malloc(sizeof(cline)*E);
    }
...
/*                                                             */
/***************************************************************/
void access_cache(cache *L, char* op, uint32_t addr,int* read, int *write, int* readhit, int *writehit, int *wb)
{
    
    time++;
    uint32_t now_index = (addr >> 3) & 0xf;
    uint32_t now_tag = (addr >> (L->s + L->b));
    ...
    if(!strcmp(op, "R")){
        ...
        // CASE1 : READ HIT
        for(int i = 0; i < L->E; i++){
            if(L->sets[now_index].lines[i].valid == 1 && L->sets[now_index].lines[i].tag == now_tag){
                L->sets[now_index].lines[i].age = time;
                (*readhit)++;
                return;
            }
        }
        // CASE2 : NOT FULL LOAD DATA
        for(int i = 0; i < L->E; i++){
            if(L->sets[now_index].lines[i].valid == 0){
                L->sets[now_index].lines[i].age = time;
                L->sets[now_index].lines[i].modified = 0;
                L->sets[now_index].lines[i].tag = now_tag;
                L->sets[now_index].lines[i].valid = 1;
                return;
            }
        }
        // CASE3 : FULL LOAD DATA
        int min = 100000000; int min_index =0;
        for(int i = 0; i < L->E; i++){
            if(L->sets[now_index].lines[i].valid == 1 && L->sets[now_index].lines[i].age < min){
                min = L->sets[now_index].lines[i].age;
                min_index = i;
            }
        }
        if(L->sets[now_index].lines[min_index].modified == 1){(*wb)++;}
        L->sets[now_index].lines[min_index].age = time;
        L->sets[now_index].lines[min_index].modified = 0;
        L->sets[now_index].lines[min_index].tag = now_tag;
        L->sets[now_index].lines[min_index].valid = 1; 
        return;
    }
    else if(!strcmp(op, "W")){
        (*write)++;
        ...
        for(int i = 0; i < L->E; i++){
            if(L->sets[now_index].lines[i].valid == 1 && L->sets[now_index].lines[i].tag == now_tag){
                L->sets[now_index].lines[i].modified = 1;
                L->sets[now_index].lines[i].age = time;
                (*writehit)++;
                return;
            }
        }
        // CASE2: WRITE ALLOCATE
        for(int i = 0; i < L->E; i++){
            if(L->sets[now_index].lines[i].valid == 0){
                L->sets[now_index].lines[i].age = time;
                L->sets[now_index].lines[i].modified = 1;
        ...
        if(L->sets[now_index].lines[min_index].modified == 1){(*wb)++;}
        L->sets[now_index].lines[min_index].age = time;
        L->sets[now_index].lines[min_index].modified = 1;
        L->sets[now_index].lines[min_index].tag = now_tag;
        L->sets[now_index].lines[min_index].valid = 1; 
        return;
    }
...
#endif
        access_cache(&simCache, op, addr, &read, &write, &readhit,  &writehit, &writeback);
    }

    // test example
    sdump(read, write, writeback, readhit, writehit, read - readhit, write- writehit);
    if (xflag) {
        xdump(&simCache);
    }
