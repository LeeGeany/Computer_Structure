...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) {
	...
	for(  i =0; i < (num_token-1); i++){
		bool is_create = false; // file you true
		directory_t* d_ret = find_create_dir(now, token_list[i], &is_create);
		if(is_create){
			now->dir_list[(now->num_dir)] = d_ret;
			(now->num_dir) = (now->num_dir) + 1;
		}
		now = d_ret;
	}
	//printf("check1\n");
	bool f_is_create = false;
	file_t* f_ret = find_create_file(now, token_list[i], &f_is_create);
	if(f_is_create){
		now->file_list[(now->num_file)] = f_ret;
		(now->num_file) = (now->num_file) + 1;
	}
	//printf("check2\n");
